...
# this is the name of the file generated by the openai_script_pt.py script
...
def get_summary(word_list):
    ...
    while l < len(word_list):
        ...
        while w < len(word_list[l]):
            if "resumo" in word_list[l][w].lower():
...
def categorize_words(word_list):
    ...
    companies = [
        ...
        and "não há" not in c
        and "não se" not in c
        and "nenhuma" not in c
        and "n/a" not in c
        and "não mencionadas" not in c
    ]
